...
class Data:
    ...
    class User(dbBaseClass):
        ...
        def __init__(self, json : dict = {}, cript = False, **kwargs):
            ...
            self.table = Data.Users_Table
            if(cript):
                self.pw = Data.User.__cryptCredentials__(self.pw, self.email) if hasattr(self, 'pw') and hasattr(self, 'email') else None


        def update(self, **kwargs):
            if('PW' in kwargs.keys()):
                kwargs['PW'] = Data.User.__cryptCredentials__(kwargs['PW'], self.email if 'EMAIL' not in kwargs.keys() else kwargs['EMAIL'])
            where_conditions = [getattr(self.table.c, key) == value for key, value in self.__initializedKeys__().items() if type(value) != Table]
            with engine.connect() as Base:
                result = Base.execute(update(self.table)
                                      .where(*where_conditions)
    ...
    class Session(dbBaseClass):
        ...
        def __init__(self, json : dict = {},**kwargs) -> None:
            super().__init__(json, **kwargs)
            self.table = Data.Users_Sessions_Table
    ...
    class Redirect:
        ...
        def getSettings():
            if(len(Data.Redirect.settings) == 0):
                Data.Redirect.__initSettings__()
            return Data.Redirect.settings

        def __initSettings__():
            with open(Data.Redirect.filePath) as file:
                Data.Redirect.settings = json.load(file)
